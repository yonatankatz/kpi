...
let goalMeasurement = (description, DesId, editId, deleteId, type, date, className, id, editEvent, removeEvent, number, actual) => {
    let date1, classDate;
    ...
    div.classList.add("d-md-flex", "p-2", "border_light", "rounded_5", "border_left", "mb-3", "align_center", className, 'w-100', actual == 'done' ? 'disableDiv' : 'a')
    ...
    // if(type == 'flag' && date != ''){
        ...
        span.classList.add('bg_orange_light', 'text-white', 'rounded-pill', 'p-1', 'text-center', 'width-10', classDate);
...
let departmentList = (id, name, deptId, paddingClass, child) => {
    ...
    list.classList.add('py-2', 'pr-2', 'position-relative', 'radioButton', paddingClass, 'dir', 'mr-2', 'width-10_9');
    ...
    label.classList.add('px-2', 'd-flex', 'align_center', 'mb-0', 'rounded_5', 'border-0', 'text-left', 'height-2_5', 'bgDropdown', 'deptHover');
...
let gridView = (goalDesc, description, measureId, percentage, id, type, actual, icon, completed, rate, deptId) => {
    let div = document.createElement('div');
    ...
    div1.appendChild(p);
    let div2 = document.createElement('div');
    div2.classList.add('p-3', 'bg-white', 'border_bottom', 'test', 'position-relative');
...
let listView = (goalId, goalDes, editGoal, removeGoal, editId, removeId, noBorder, pl0, border0) => {
    ...
    let div = document.createElement('div');
    div.classList.add('d-md-flex', 'p-3', 'border_light', 'rounded_5', 'bg-white', 'w-100', 'align_center');
    div.setAttribute('id', 'goal-id'+goalId);
    let div1 = document.createElement('div');
    div1.classList.add('mr-auto', 'img_center', 'bg-transparent', 'cursor_pointer', 'w-75', 'p-2');
...
let listMeasurementGoal = (goalId, description, type, date, compBtn) => {
    ...
    let div = document.createElement('div');
    div.classList.add("d-md-flex", "p-1", "p-sm-3", "border_light", "rounded_5", "border_left", "bg-white", "align_center", 'position-relative')
    let div1 = document.createElement('div')
    ...
    if(compBtn == 'done'){
        ...
        if(date != undefined && date !=''){
            ...
            p1.classList.add('bg_orange_light', 'text-white', 'rounded-pill', 'p-2', 'text-center', 'width-10', 'mb-0', 'font_light');
...
let actionList = (text, id, done) => {
    let div = document.createElement('div');
    div.classList.add('row', 'mb-3', 'd-flex', 'align_center', 'actionProgressList');
    let div1 = document.createElement('div');
    ...
    let div2 = document.createElement('div');
    div2.classList.add('col-md-3', 'd-flex', 'justify-content-end', 'justify-content-md-center', 'actionCheckbox');
    let input = document.createElement('input');
    ...
    label.classList.add('bg_white', 'border-0', 'width-6', 'height-2_5', 'rounded_5', 'buttonAdd', 'align_center', 'p-2', 'text-center', 'mb-0');
    if(done == 'done'){
        let fa = document.createElement('i');
...
let addNewGoalFunc = (array, deptId, goalId) => {
    ...
    if(utils.storage.get(deptId)){
        if(goalId == 'parentGoal'){
            ...
            let ary = utils.storage.get(utils.storage.get('department'+deptId).parentId);
            ary.map((item, index) => {
        ...
        set[deptId] = array1;
        if(utils.storage.get(utils.storage.get('department'+deptId).parentId) && goalId != 'parentGoal'){
            let ary = utils.storage.get(utils.storage.get('department'+deptId).parentId);
            ary.map((item, index) => {
                if(item.goal.goalId == parseInt(goalId)){
                    if(item.children){
