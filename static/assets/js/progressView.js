...
let gridViewFunc = (result) => {
    result.map( (item, index) => {
        ...
        item.measurements.map( (item1, index1) => {
            ...
            let deptIdScroll = parseInt($('input[name=radio]:checked', '#DeptForm').val());

            if(item1.type == 'number'){
                if(item1.actual == 'done'){
                    gridViewId.appendChild(gridView(item.goal.description, item1.description, item1.measureId, 100, item.goal.goalId, item1.type, '', '','100% complete', 'Current rate: '+(100).toFixed(0), deptIdScroll));
                    chartUI('doughnut'+item1.measureId, [100, 0]);
                }
                else{
                    let percentage = ((100*(item1.actual))/item1.target);
                    gridViewId.appendChild(gridView(item.goal.description, item1.description, item1.measureId, percentage, item.goal.goalId, item1.type, '', '', (percentage).toFixed(0)+'% Complete', 'Current rate: '+(percentage).toFixed(0), deptIdScroll));
                    chartUI('doughnut'+item1.measureId, [percentage, 100-percentage]);
                }
            }
            if(item1.type == 'flag'){
                if(item1.actual == 'done'){
                    gridViewId.appendChild(gridView(item.goal.description, item1.description, item1.measureId, '', item.goal.goalId, item1.type, item1.actual, 'fa-thumbs-up', '100% Complete', 'Current progress: '+actionComplete+'/'+totalActions, deptIdScroll));
                    chartUI('doughnut'+item1.measureId, [100, 0]);
                }
                else{
                    if(milliseconds(new Date(item1.dueDate)) >= milliseconds(new Date())){
                        gridViewId.appendChild(gridView(item.goal.description, item1.description, item1.measureId, '', item.goal.goalId, item1.type, '', 'fa-spinner', '0% Complete', 'Current progress: '+actionComplete+'/'+totalActions, deptIdScroll));
                        chartUI('doughnut'+item1.measureId, [0, 100]);
                    }
                    else if(milliseconds(new Date(item1.dueDate)) < milliseconds(new Date())){
                        gridViewId.appendChild(gridView(item.goal.description, item1.description, item1.measureId, '', item.goal.goalId, item1.type, '', 'fa-thumbs-down', '0% Complete', 'Current progress: '+actionComplete+'/'+totalActions, deptIdScroll));
                        chartUI('doughnut'+item1.measureId, [0, 100]);
                    }
                    else{
                        gridViewId.appendChild(gridView(item.goal.description, item1.description, item1.measureId, '', item.goal.goalId, item1.type, '', 'fa-spinner', '0% Complete', 'Current progress: '+actionComplete+'/'+totalActions, deptIdScroll));
        ...
        if(item.measureId == measureId){
            ...
            if(item.type == 'number'){
                ...
                if(item.actual == 'done'){
                    ...
                    (document.getElementById('flagKPI')).nextElementSibling.classList.add('cursorEvent', 'bg-white');
                }
        ...
        actionObj.map( (item, index) => {
            if(item.goalId == progressGoalId){
                ...
                if(item.done == 'done'){
                    ...
                    let actionBtn = document.getElementById('actionProgress'+item.actionId);
...
let classFunction = () => {
    ...
    (document.getElementById('flagKPI')).nextElementSibling.classList.remove('cursorEvent', 'bg-white');
}
...
$('#saveProgress').click( (e) => {
    ...
    let edit = (result) => {
        ...
        $('.actionProgressList').each( (i, obj) => {
            let action = {
                actionId : obj.firstElementChild.firstElementChild.firstElementChild.getAttribute('id').replace('actionPr', ''),
                text : obj.firstElementChild.firstElementChild.firstElementChild.textContent,
            }
            if(document.getElementById(obj.lastElementChild.firstElementChild.getAttribute('id')).checked == true){
                action.done = 'done'
            }
        ...
        let editGoal = async() => {
            try{
                const goalData = await api.editGoal(result, $('input[name=radio]:checked', '#DeptForm').val(), true, progressGoalId);
