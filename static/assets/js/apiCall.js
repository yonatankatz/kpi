...
const local = {
    ...
    init: (returnFunc) => {
        ...
        API('/api/get-all', '', 'GET').done(function(response) {
    ...
    updateChangeLog: (changeLog) => {
        ...
        if (set) {
            for (var prop in set) {
                if (Object.prototype.hasOwnProperty.call(set, prop)) {
                    API('/api/?filename=' + prop, JSON.stringify(set[prop]), 'POST').done(function (response) {
        ...
        if (deleted) {
            for (var i = 0; i < deleted.length; i++) {
                API('/api/?filename=' + deleted[i], '', 'DELETE').done(function(response) {
    ...
    setDepartment: (id, array) => {
        return new Promise((resolve) => {
            API(`/api/?filename=department${id}`, JSON.stringify(array), 'POST').done(function(response) {
    ...
    setGoalArray: (id, array) => {
        return new Promise((resolve) => {
            API(`/api/?filename=${id}`, JSON.stringify(array), 'POST').done(function(response) {
    ...
    getGoalListOfParentDept: (deptId) => {
        return new Promise((resolve) => {
            setTimeout( () => resolve(utils.storage.get(utils.storage.get('department'+deptId).parentId)), timeOut);
        })
    },

    getParentDepartmentId: (id) => {
        return new Promise((resolve) => {
            setTimeout( () => resolve(utils.storage.get('department'+id).parentId), timeOut);
        })
    },
    ...
    setEditGoalArray: (id) => {
        const changeLog = {'set' : {'editGoalDetail' : utils.storage.get('setGoal'+id)}};
        this.api.updateChangeLog(changeLog);
        return new Promise((resolve) => {
            setTimeout( () => {
                resolve(utils.storage.set('editGoalDetail', utils.storage.get('setGoal'+id)));
            }, 500);
        })
    }
